...
int64_t StreamWriter2::default_row_group_size_{512 * 1024 * 1024};  // 512MB

constexpr int16_t StreamWriter2::kDefLevelZero;
constexpr int16_t StreamWriter2::kDefLevelOne;
constexpr int16_t StreamWriter2::kRepLevelZero;
constexpr int64_t StreamWriter2::kBatchSizeOne;

StreamWriter2::FixedStringView::FixedStringView(const char* data_ptr)
    : data{data_ptr}, size{std::strlen(data_ptr)} {}

StreamWriter2::FixedStringView::FixedStringView(const char* data_ptr, std::size_t data_len)
    : data{data_ptr}, size{data_len} {}

StreamWriter2::StreamWriter2(std::unique_ptr<ParquetFileWriter> writer)
...
void StreamWriter2::SetDefaultMaxRowGroupSize(int64_t max_size) {
  default_row_group_size_ = max_size;
}

void StreamWriter2::SetMaxRowGroupSize(int64_t max_size) {
  max_row_group_size_ = max_size;
}

int StreamWriter2::num_columns() const { return static_cast<int>(nodes_.size()); }

StreamWriter2& StreamWriter2::operator<<(bool v) {
  CheckColumn(Type::BOOLEAN, ConvertedType::NONE);
  return Write<BoolWriter>(v);
}

StreamWriter2& StreamWriter2::operator<<(int8_t v) {
  CheckColumn(Type::INT32, ConvertedType::INT_8);
  return Write<Int32Writer>(static_cast<int32_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(uint8_t v) {
  CheckColumn(Type::INT32, ConvertedType::UINT_8);
  return Write<Int32Writer>(static_cast<int32_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(int16_t v) {
  CheckColumn(Type::INT32, ConvertedType::INT_16);
  return Write<Int32Writer>(static_cast<int32_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(uint16_t v) {
  CheckColumn(Type::INT32, ConvertedType::UINT_16);
  return Write<Int32Writer>(static_cast<int32_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(int32_t v) {
  CheckColumn(Type::INT32, ConvertedType::INT_32);
  return Write<Int32Writer>(v);
}

StreamWriter2& StreamWriter2::operator<<(uint32_t v) {
  CheckColumn(Type::INT32, ConvertedType::UINT_32);
  return Write<Int32Writer>(static_cast<int32_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(int64_t v) {
  CheckColumn(Type::INT64, ConvertedType::INT_64);
  return Write<Int64Writer>(v);
}

StreamWriter2& StreamWriter2::operator<<(uint64_t v) {
  CheckColumn(Type::INT64, ConvertedType::UINT_64);
  return Write<Int64Writer>(static_cast<int64_t>(v));
}

StreamWriter2& StreamWriter2::operator<<(const std::chrono::milliseconds& v) {
  CheckColumn(Type::INT64, ConvertedType::TIMESTAMP_MILLIS);
  return Write<Int64Writer>(static_cast<int64_t>(v.count()));
}

StreamWriter2& StreamWriter2::operator<<(const std::chrono::microseconds& v) {
  CheckColumn(Type::INT64, ConvertedType::TIMESTAMP_MICROS);
  return Write<Int64Writer>(static_cast<int64_t>(v.count()));
}

StreamWriter2& StreamWriter2::operator<<(float v) {
  CheckColumn(Type::FLOAT, ConvertedType::NONE);
  return Write<FloatWriter>(v);
}

StreamWriter2& StreamWriter2::operator<<(double v) {
  CheckColumn(Type::DOUBLE, ConvertedType::NONE);
  return Write<DoubleWriter>(v);
}

StreamWriter2& StreamWriter2::operator<<(char v) { return WriteFixedLength(&v, 1); }

StreamWriter2& StreamWriter2::operator<<(FixedStringView v) {
  return WriteFixedLength(v.data, v.size);
}

StreamWriter2& StreamWriter2::operator<<(const char* v) {
  return WriteVariableLength(v, std::strlen(v));
}

StreamWriter2& StreamWriter2::operator<<(const std::string& v) {
  return WriteVariableLength(v.data(), v.size());
}

StreamWriter2& StreamWriter2::operator<<(arrow::util::string_view v) {
  return WriteVariableLength(v.data(), v.size());
}

StreamWriter2& StreamWriter2::WriteVariableLength(const char* data_ptr,
                                                std::size_t data_len) {
...
void StreamWriter2::CheckColumn(Type::type physical_type,
...
int64_t StreamWriter2::SkipColumns(int num_columns_to_skip) {
...
void StreamWriter2::SkipOptionalColumn() {
  if (SkipColumns(1) != 1) {
...
void StreamWriter2::EndRow() {
  if (!file_writer_) {
    throw ParquetException("StreamWriter2 not initialized");
  }
  if (static_cast<std::size_t>(column_index_) < nodes_.size()) {
...
void StreamWriter2::EndRowGroup() {
  if (!file_writer_) {
    throw ParquetException("StreamWriter2 not initialized");
  }
