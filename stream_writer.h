...
class StreamWriter2 {
 public:
  ...
  //      assigned afterwards.
  StreamWriter2() = default;

  explicit StreamWriter2(std::unique_ptr<ParquetFileWriter> writer);
  ...
  // Moving is possible.
  StreamWriter2(StreamWriter2&&) = default;
  StreamWriter2& operator=(StreamWriter2&&) = default;

  // Copying is not allowed.
  StreamWriter2(const StreamWriter2&) = delete;
  StreamWriter2& operator=(const StreamWriter2&) = delete;

  /// \brief Output operators for required fields.
  /// These can also be used for optional fields when a value must be set.
  StreamWriter2& operator<<(bool v);

  StreamWriter2& operator<<(int8_t v);

  StreamWriter2& operator<<(uint8_t v);

  StreamWriter2& operator<<(int16_t v);

  StreamWriter2& operator<<(uint16_t v);

  StreamWriter2& operator<<(int32_t v);

  StreamWriter2& operator<<(uint32_t v);

  StreamWriter2& operator<<(int64_t v);

  StreamWriter2& operator<<(uint64_t v);

  StreamWriter2& operator<<(const std::chrono::milliseconds& v);

  StreamWriter2& operator<<(const std::chrono::microseconds& v);

  StreamWriter2& operator<<(float v);

  StreamWriter2& operator<<(double v);

  StreamWriter2& operator<<(char v);
  ...
  template <int N>
  StreamWriter2& operator<<(const char (&v)[N]) {
    return WriteFixedLength(v, N);
  }
  template <std::size_t N>
  StreamWriter2& operator<<(const std::array<char, N>& v) {
    return WriteFixedLength(v.data(), N);
  }
  StreamWriter2& operator<<(FixedStringView v);

  /// \brief Output operators for variable length strings.
  StreamWriter2& operator<<(const char* v);
  StreamWriter2& operator<<(const std::string& v);
